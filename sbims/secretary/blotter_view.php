...
Auth::checkAuth();
Auth::checkRole(['secretary', 'captain']);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_status'])) {
    ...
    $update_query = "UPDATE blotters SET status = :status, resolution = :resolution";
    
    if ($new_status == 'Settled') {
        $update_query .= ", resolved_date = NOW()";
    }
    ...
    $update_stmt->bindParam(':resolution', $resolution);
    $update_stmt->bindParam(':id', $blotter_id);
    
    if ($update_stmt->execute()) {
        Auth::logActivity($_SESSION['user_id'], 'Update Blotter', "Updated blotter case {$blotter['case_id']} status to $new_status");
    ...
    <main class="content">
        <div class="page-header">
            ...
            <div class="page-actions">
                <a href="blotter.php" class="btn btn-outline">Back to List</a>
                <?php if ($_SESSION['user_role'] == 'secretary'): ?>
                    <a href="blotter_edit.php?id=<?php echo $blotter_id; ?>" class="btn btn-primary">Edit Case</a>
                <?php endif; ?>
            </div>
        </div>
            ...
            <div class="detail-section">
                ...
                <form method="POST" class="status-form">
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <select id="status" name="status" required>
                                ...
                                <option value="Referred" <?php echo $blotter['status'] == 'Referred' ? 'selected' : ''; ?>>Referred</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="resolution" name="resolution" rows="4" placeholder="Enter resolution details when case is settled or referred..."><?php echo htmlspecialchars($blotter['resolution'] ?? ''); ?></textarea>
                    </div>
